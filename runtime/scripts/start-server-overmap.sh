...
dune_systemctl stop dune-server-overmap.service 2>/dev/null || true
engine rm -f dune-server-overmap 2>/dev/null || true

ensure_quadlet_foundation

quadlet_write dune-server-overmap.container <<EOF
# Generated by runtime/scripts/start-server-overmap.sh. Do not edit by hand.
[Unit]
Description=Dune Awakening game server (Overmap)

[Container]
ContainerName=dune-server-overmap
Image=${IMAGE}
Pull=never
Network=host
AddCapability=SYS_ADMIN
PodmanArgs=--privileged --security-opt=seccomp=unconfined --memory=${MEMORY} --memory-reservation=${MEMORY}
Volume=${REPO_DIR}/runtime/game/overmap/Saved:/home/dune/server/DuneSandbox/Saved:z
...
Environment="POD_UID=docker-overmap"
Environment="POD_NAME=${BATTLEGROUP_ID}-sg-overmap-pod-2"
Environment="POD_IP=${MULTIHOME_IP}"
Environment="EXTERNAL_ADDRESS_OVERRIDE=${SERVER_IP}"
Environment="NODE_NAME=$(hostname)"
Environment="SERVER_INDEX=2"
...
Exec=/opt/dune-local/run-server.sh Overmap "-FarmRegion=${SERVER_REGION}" "-ini:engine:[FuncomLiveServices]:ServiceAuthToken=${FUNCOM_TOKEN}" -RMQGameTlsEnabled=true "ServerName=${BATTLEGROUP_ID}" "-MultiHome=${MULTIHOME_IP}" -DatabaseName=dune -DatabaseHost=127.0.0.1:15432 -DatabaseUser=dune -DatabasePassword=dune "-PartitionIndex=${PARTITION_ID}" "-ini:engine:[URL]:Port=${GAME_PORT}" "-ini:engine:[URL]:IGWPort=${IGW_PORT}" -battlegroup-director-url=127.0.0.1:11717 --RMQGameHostname=127.0.0.1 --RMQGamePort=31982 --RMQAdminHostname=127.0.0.1 --RMQAdminPort=32573 ${SIETCH_RUNTIME_ARGS[*]} ${LOG_RUNTIME_ARGS[*]}
...
quadlet_reload
dune_systemctl start dune-server-overmap.service
...
engine ps --filter "name=dune-server-overmap" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"

echo
echo "=== overmap logs ==="
engine logs --tail 180 dune-server-overmap
